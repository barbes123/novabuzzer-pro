              ...
              {players.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {players.map((p, idx) => {
                    ...
                    return (
                      <div key={p.id} className={`p-5 rounded-2xl flex items-center justify-between group transition-all shadow-lg border-2 ${
                        isWinner ? 'bg-yellow-500/10 border-yellow-500 shadow-[0_0_30px_rgba(234,179,8,0.2)]' : 
                        p.disabled ? 'bg-slate-900 border-slate-800 opacity-40' : 'bg-slate-800/50 border-slate-700 hover:border-blue-500'
                      }`}>
                        <div className="flex items-center gap-4">
                          <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-xl italic ${isWinner ? 'bg-yellow-500 text-slate-950' : 'bg-blue-600/10 text-blue-400'}`}>
                            {idx + 1}
                          </div>
                          <div className="flex flex-col">
                            ...
                            <div className="flex items-center gap-2">
                               <span className="text-[8px] font-mono text-slate-500 uppercase tracking-widest">{t.PLAYER_LABEL} {idx + 1}</span>
                               {p.disabled && <span className="text-[8px] font-black text-red-500 uppercase tracking-widest">{t.PAUSED}</span>}
                               {isWinner && <span className="text-[8px] font-black text-yellow-500 uppercase tracking-widest">{t.CHAMPION}</span>}
                            </div>
                          </div>
                        </div>
                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleAction('TOGGLE_PAUSE', { id: p.id })} title={p.disabled ? 'Resume' : 'Pause'} className={`p-2 rounded-lg transition-colors ${p.disabled ? 'bg-green-900/20 text-green-500' : 'bg-orange-900/20 text-orange-500'}`}>
                          ...
                          <button onClick={() => { setEditingId(p.id); setTempName(p.name); }} className="p-2 bg-slate-700 rounded-lg hover:bg-slate-600 transition-colors">
                            <svg className="w-4 h-4 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                          </button>
