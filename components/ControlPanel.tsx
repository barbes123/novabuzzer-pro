...
const ControlPanel: React.FC<Props> = ({ socket, gameState, gateCode, players, buzzes, language }) => {
  ...
  const handleAction = (type: string, data?: any) => {
    ...
    // We calculate the fresh status inside the timeout
    setTimeout(() => {
      // Use the logic directly here to ensure it's fresh
      const currentIsArmed = gameState === 'ACTIVE' || gameState === 'BATTLE';
      setLastAction(currentIsArmed ? 'Armed' : 'Standby');
    }, 1000);
  };
      ...
      <header className="bg-slate-900/50 backdrop-blur-xl border-b border-slate-800 p-4 flex items-center justify-between sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 bg-slate-800 border border-slate-700 px-4 py-2 rounded-xl">
            ...
            <input type="range" min="0" max="1" step="0.01" value={volume} onChange={(e) => setVolume(parseFloat(e.target.value))} className="w-20 h-1.5 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-blue-500" />
            <span className="text-[10px] font-mono font-bold text-slate-500 w-8">{Math.round(volume * 100)}%</span>
          </div>
      ...
      <main className="flex-1 overflow-auto p-6 md:p-10">
        {activeTab === 'host' ? (
          <div className="max-w-5xl mx-auto space-y-8">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button onClick={copyGateCode} className="p-4 bg-slate-800 hover:bg-slate-700 rounded-2xl text-slate-400 hover:text-white transition-all border border-slate-700 flex items-center gap-2">
                    {copied ? (
                      ...
                      <><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 16h8M8 12h8m-6 8h6a2 2 0 002-2V8a2 2 0 00-2-2h-6l-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2h2z" /></svg><span className="text-[10px] font-black uppercase tracking-widest">Copy</span></>
                    )}
                  </button>
                </div>
              </div>
              ...
              <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl shadow-2xl flex flex-col justify-center">
                ...
                <div className="flex flex-col gap-2">
                  <div className={`text-3xl font-black italic tracking-tighter uppercase transition-colors ${isArmed ? 'text-green-400' : gameState === 'WINDOW_OPEN' ? 'text-yellow-400 animate-pulse' : 'text-slate-600'}`}>
                    {gameState}
                  </div>
            ...
            {/* MAIN BANNER: Updated to respond to BATTLE state */}
            <div className={`p-10 rounded-[3rem] shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8 transition-all duration-500 ${isArmed ? 'bg-red-600 animate-pulse shadow-[0_0_50px_rgba(220,38,38,0.3)]' : 'bg-blue-600'}`}>
              <div className="text-center md:text-left">
                <h3 className="text-4xl font-black text-white uppercase italic tracking-tighter mb-2">
                  {isArmed ? t.ARMED : t.READY}
                </h3>
                <p className="text-white opacity-70 max-w-md text-xs font-bold uppercase tracking-widest">
                  {isArmed ? t.ARM_DESC : t.READY_DESC}
                </p>
              </div>
              <div className="flex gap-4">
                <button
                  // Disable if NOT Idle (meaning it's already ACTIVE or BATTLE)
                ...
                <button
                  // Enable if NOT Idle (allows you to stop a BATTLE or ACTIVE session)
                  disabled={gameState === 'IDLE'}
            ...
            <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl shadow-2xl">
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex gap-4 items-center">
                  <button onClick={() => handleAction('KICK_ALL')} className="px-4 py-2 bg-red-900/20 text-red-500 border border-red-900/30 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-red-900/40 transition-all">{t.KICK_ALL}</button>
              ...
              {players.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {players.map((p, idx) => {
                    ...
                    return (
                      <div key={p.id} className={`p-5 rounded-2xl flex flex-col group transition-all shadow-lg border-2 ${isWinner ? 'bg-yellow-500/10 border-yellow-500 shadow-[0_0_30px_rgba(234,179,8,0.2)]' : p.disabled ? 'bg-slate-900 border-slate-800 opacity-60' : 'bg-slate-800/50 border-slate-700 hover:border-blue-500'}`}>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-xl italic ${isWinner ? 'bg-yellow-500 text-slate-950' : 'bg-blue-600/10 text-blue-400'}`}>{idx + 1}</div>
                            <div className="flex flex-col">
                              {editingId === p.id ? (
                                ...
                                <span className={`font-black uppercase tracking-tight truncate text-lg ${isWinner ? 'text-yellow-500' : 'text-slate-100'}`}>{p.name}</span>
                              )}
                          ...
                          <div className="flex gap-2">
                            <button onClick={() => handleAction('TOGGLE_PAUSE', { id: p.id })} className={`p-2 rounded-lg transition-colors ${p.disabled ? 'bg-green-900/20 text-green-500 hover:bg-green-900/40' : 'bg-orange-900/20 text-orange-500 hover:bg-orange-900/40'}`}><svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zM7 8a1 1 0 012 0v4a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v4a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" /></svg></button>
                            <button onClick={() => { setEditingId(p.id); setTempName(p.name); }} className="p-2 bg-slate-700 rounded-lg hover:bg-slate-600 transition-colors"><svg className="w-4 h-4 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg></button>
                            <button onClick={() => handleAction('KICK_PLAYER', { id: p.id })} className="p-2 bg-red-900/20 rounded-lg hover:bg-red-900/40 transition-colors"><svg className="w-4 h-4 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 mt-3 pt-2 border-t border-slate-700/50">
                          ...
                          {!isWinner && !p.disabled && <span className="text-[10px] text-slate-600 uppercase tracking-widest">ACTIVE</span>}
                        </div>
                      </div>
                    );
                  })}
                    ...
                    {buzzes.map((b, idx) => (
                      <div key={idx} className={`p-8 flex items-center justify-between transition-all group ${idx === 0 ? 'bg-yellow-500/5 border-l-8 border-yellow-500' : 'hover:bg-slate-800/30 border-l-8 border-transparent'}`}>
                        ...
                        <div className="text-right">
                          ...
                          {idx > 0 && <div className="text-[10px] font-mono text-red-500 mt-1 uppercase font-bold tracking-tighter">Gap: +{b.offset - buzzes[0].offset}ms</div>}
                        </div>
                      </div>
                    ))}
                  </div>
